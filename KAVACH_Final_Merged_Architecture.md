...
| **OCR pipeline** — pypdf (digital text) + Tesseract OCR / pytesseract (text + per-word confidence) | `backend/tools/ocr.py` | **P0 — PS multimodal** |
...
| OCR / Vision | **pypdf + Tesseract OCR (`pytesseract`) + Qwen2.5-VL** | NEW |
...
**4.2 OCR + vision (P0, the biggest net-new).** Documents are handled via a two-tier extraction pipeline: `pypdf` extracts digital text layers directly from searchable PDFs with 100% confidence; for scanned images and documents, **Tesseract OCR (`pytesseract`)** serves as the primary engine, extracting text with real per-word confidence scores via `image_to_data` and flagging low-confidence results (<65%) for human review. (PaddleOCR was evaluated during early phases but abandoned due to an upstream oneDNN C++ crash on Windows; see KAVACH_CONTEXT.md line 216). For drawings/photos, the image goes to vision models with structured inspection questions. Honest scope: assisted understanding, confidence shown, low-confidence flagged for human. This feeds the modified `ingest.py` and the OCR/vision agent tool. **This is a key multimodal addition.**
...
Two things carry the finale and neither exists in Alok's code yet: **the sovereignty proof** and **multimodal OCR/vision**. Everything else you're mostly inheriting. So do not let the comfort of a working skeleton lull you into polishing what already works — put your scarce build time on those two. (Note: While PaddleOCR was initially considered, it was replaced by Tesseract OCR (`pytesseract`) due to fatal oneDNN compatibility crashes on Windows; ensure Tesseract binaries and language data are verified on the demo machine).
